...
// ==================== 拡張版ExpenseManager ====================
class CloudEnabledExpenseManager extends ExpenseManager {
    constructor() {
        super();
    ...
    async initGoogleSync() {
        // Google Drive同期を初期化
        this.googleSync = new GoogleDriveSync();
        await this.googleSync.init();
    }
    ...
    async saveExpense() {
        ...
        // Google Driveに同期
        if (this.googleSync && this.googleSync.isSignedIn) {
            // 少し遅延させて同期（UIの更新を優先）
            setTimeout(() => {
                this.googleSync.syncData();
            }, 500);
        }
    }
...
document.addEventListener('DOMContentLoaded', async () => {
    // CloudEnabledExpenseManagerを使用
    expenseManager = new CloudEnabledExpenseManager();
